...
<body class="bg-slate-50 font-sans text-slate-900 antialiased">
    ...
    <main class="w-full max-w-full p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            @forelse($tasks as $task)
                <div
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($task->deadline)
                                <span class="text-xs text-red-500 font-medium">
                                    ⏳ Batas:
                                    {{ \Carbon\Carbon::parse($task->deadline)->translatedFormat('d M Y, H:i') }} WITA
                                </span>
                            @endif

                            @if ($task->status_verifikasi === 'acc')
                                <span
                                    class="inline-flex items-center rounded-md bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700 border border-emerald-100">
                                    Disetujui Admin
                                </span>
                                @------------ SINKRONISASI EXPIRED STATUS -------------
                            @elseif($task->status_verifikasi === 'kedaluwarsa' || ($task->deadline && now()->greaterThan($task->deadline)))
                                <span
                                    class="inline-flex items-center rounded-md bg-rose-50 px-2 py-0.5 text-xs font-semibold text-rose-700 border border-rose-100">
                                    ❌ Waktu Akses Habis
                                </span>
                                @------------------------------------------------------
                            @elseif($task->status_verifikasi === 'menunggu' && $task->url_status == 1)
                                <span
                                    class="inline-flex items-center rounded-md bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 border border-blue-100">
                                    Menunggu Verifikasi
                                </span>
                            @elseif($task->status_verifikasi === 'tolak')
                                <span
                                    class="inline-flex items-center rounded-md bg-red-50 px-2 py-0.5 text-xs font-medium text-red-700 border border-red-100">
                                    Ditolak (Mohon Upload Ulang)
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center rounded-md bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500 border border-slate-200">
                                    Belum Dikerjakan
                                </span>
                            @endif
                        </div>
                    ...
                    <div
                        ...
                        @elseif($task->status_verifikasi === 'kedaluwarsa' || ($task->deadline && now()->greaterThan($task->deadline)))
                            <div
                                class="text-sm font-semibold text-rose-600 bg-rose-50 border border-rose-100 px-4 py-2 rounded-lg text-center">
                                🔒 Pengunggahan Ditutup (Expired)
                            </div>
                        @else
                            <form action="{{ route('pegawai.task.upload', $task->id) }}" method="POST"
                                ...
                                @error('deadline')
                                    <p class="text-xs text-red-500 mb-1 block sm:hidden">{{ $message }}</p>
                                @enderror
