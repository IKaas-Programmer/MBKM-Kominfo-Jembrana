                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($recentTrackings as $tracking)
                            <tr class="hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="px-6 py-4 text-slate-600 max-w-xs truncate">{{ $tracking->nama_tugas }}</td>
                                <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ $tracking->kode_unik }}</td>
                                <td class="px-6 py-4">
                                    @if ($tracking->status_verifikasi === 'menunggu')
                                        <span
                                            class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 border border-blue-200">Menunggu</span>
